...
- Discord OAuth 접근 제한: `identify guilds` 권한으로 관리 가능한 서버만 표시
...
- 선택 AI: OpenAI, Anthropic, Gemini provider를 선택 옵션으로 지원
- 리포트 캐싱/큐: guild+period cacheKey로 재사용, Redis/BullMQ 큐 확장
- Redis rate limit: 설정 저장, 리포트 생성, 데이터 삭제 API 보호
...
Discord OAuth 대시보드를 쓰려면 다음 값도 필요합니다.
...
- `/embedded`: Discord Embedded App SDK ready flow 기본 페이지
...
- 대시보드는 Discord OAuth의 서버 권한 목록을 확인해 `Manage Guild` 또는 `Administrator` 권한이 있는 서버만 접근을 허용합니다.
- 설정 변경, 리포트 생성, 멤버 데이터 삭제 API는 Redis rate limit을 우선 사용하고 Redis가 없으면 메모리 fallback을 사용합니다.
...
외부 AI API는 기본값이 아니며 선택 옵션입니다. 로컬 LLM을 사용할 수 없거나 더 높은 품질이 필요할 때만 활성화하세요.
...
서버별 대시보드 설정에서도 provider 선택값을 저장할 수 있습니다.

## Embedded App 확장 계획

`apps/web/app/embedded/page.tsx`와 `apps/web/lib/discord-sdk.ts`는 `@discord/embedded-app-sdk`를 사용해 SDK ready flow를 수행합니다. `NEXT_PUBLIC_DISCORD_CLIENT_ID`를 설정한 뒤 Discord Embedded App 설정에서 URL mapping을 연결하면 Discord 안에서 대시보드 확장 작업을 이어갈 수 있습니다.

## 리포트 캐싱과 큐

리포트는 `guildId + cacheKey`로 캐싱됩니다. 같은 서버의 같은 기간 리포트는 재사용됩니다.

```bash
pnpm worker:reports
```

Redis가 설정되어 있으면 BullMQ 큐가 사용됩니다. Redis가 없으면 리포트 생성 요청은 즉시 생성 fallback으로 동작합니다.

## 데이터 삭제 요청 API

관리자는 대시보드 멤버 페이지 또는 API로 유저 데이터를 삭제할 수 있습니다.

```bash
DELETE /api/guilds/:guildId/members/:userId
```

요청 기록은 `DataDeletionRequest`와 `AuditLog`에 남습니다.
...
GitHub Actions 설정은 `.github/workflows/ci.yml`에 포함되어 있습니다.

## 향후 로드맵

- Discord Embedded App OAuth code exchange 고도화
- 운영용 process manager 또는 systemd 서비스
- 관리자 대시보드 실시간 차트
- 리포트 worker 스케줄러
- 세부 권한 역할 매핑
